...
.stButton > button:hover {
    ...
    box-shadow: 0 18px 35px rgba(102,126,234,0.5) !important; 
...
# Header
# Header (avec images URL en arrière-plan - SEULEMENT header)
header_code = """
<!DOCTYPE html>
<html>
<head>
...
<style>
    ...
    .overlay {
        ...
        width: 100%; 
        height: 100%;
        background: linear-gradient(rgba(10,10,16,0.4) 0%, rgba(26,26,46,0.6) 100%);
        z-index: 1; 
        pointer-events: none;
    }
    .content { 
...
<body>
    ...
    <script>
        const images = [
            "https://www.azart.fr/photo/art/grande/9724951-15680569.jpg?v=1466608504",
...
def calculate_mrp():
    ...
    for d in state['demands']:
        ...
        for art_code, qty in gross_needs.items():
            if art_code not in state['articles']:
                continue
...
if current_tab == 0:  # Dashboard
    ...
    with col1:
        st.markdown(
            f'<div class="metric-card"><div class="metric-value">{len(state["demands"])}</div><div class="metric-label">📈 Demandes</div></div>', 
            unsafe_allow_html=True
        )
    with col2:
        st.markdown(
            f'<div class="metric-card"><div class="metric-value">{len(state["articles"])}</div><div class="metric-label">📦 Articles</div></div>', 
            unsafe_allow_html=True
        )
    with col3:
        st.markdown(
            f'<div class="metric-card"><div class="metric-value">{len(state["stocks"])}</div><div class="metric-label">🏬 Stocks</div></div>', 
            unsafe_allow_html=True
        )
    with col4:
        st.markdown(
            f'<div class="metric-card"><div class="metric-value">{total_stock:.0f}</div><div class="metric-label">📊 Total Stock</div></div>', 
            unsafe_allow_html=True
        )
    ...
    if state['articles']:
        ...
        st.dataframe(
            pd.DataFrame([vars(a) for a in state['articles'].values()]), 
    ...
    with col1:
        article = st.text_input("📦 Article", key="stock_article")
    with col2:
        quantity = st.number_input("📈 Quantité", min_value=0.0, value=0.0, format="%.2f", key="stock_qty")
    with col3:
        safety_stock = st.number_input("🛡️ Stock sécurité", min_value=0.0, value=0.0, format="%.2f", key="stock_safety")
    ...
    with col1:
        parent = st.text_input("👑 Parent", key="bom_parent")
    with col2:
    ...
    if state['boms']:
        st.dataframe(
            pd.DataFrame([vars(b) for b in state['boms']]), 
            use_container_width=True
        )
    ...
    with col1:
        client = st.text_input("👥 Client", key="demand_client")
    with col2:
        article = st.text_input("📦 Article", key="demand_article")
    with col3:
        demand_qty = st.number_input("📊 Qté", value=10.0, key="demand_qty")
    with col4:
        due = st.date_input("📅 Date", value=pd.Timestamp.now().date(), key="demand_date")
    ...
    if state['demands']:
        st.dataframe(
            pd.DataFrame([vars(d) | {'due_date': d.due_date.strftime('%Y-%m-%d')} for d in state['demands']]), 
            use_container_width=True
        )
    ...
    with pd.ExcelWriter(output, engine='openpyxl') as writer:
        ...
        if state['demands']: 
            pd.DataFrame([vars(d) | {'due_date': d.due_date.strftime('%Y-%m-%d')} for d in state['demands']]).to_excel(writer, 'Demandes', index=False)
        if not state['mrp_results'].empty: 
