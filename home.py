# home.py - RESPONSIVE MOBILE + PC ✅ DARK/LIGHT ✅ SIDEBAR INVISIBLE ✅ IMAGES
import streamlit as st
import streamlit.components.v1 as components

# CSS RESPONSIVE + SIDEBAR INVISIBLE (inchangé)
st.markdown("""
<style>
    /* ✅ SIDEBAR INVISIBLE */
...
# Configuration
st.set_page_config(page_title="Logistics Intelligence", page_icon="🚚", layout="wide", initial_sidebar_state="collapsed")

# Fonction thème + CSS (identique)
def get_theme():
    try: return st.get_option("theme.base") or "light"
    except: return "light"
...
# Header responsive AVEC IMAGES (modifié)
...
<style>body{{margin:0;padding:0;background:transparent;font-family:'Inter',sans-serif;overflow:hidden;}}.main-header{{position:relative;padding:35px;background:linear-gradient(135deg,{header_bg});border-radius:24px;border-left:12px solid #667eea;overflow:hidden;box-shadow:0 25px 50px rgba(0,0,0,0.6);min-height:160px;display:flex;flex-direction:column;justify-content:center;}}@media(max-width:768px){{.main-header{{padding:20px;border-left-width:6px;min-height:120px;}}}}#bg-carousel{{position:absolute;top:0;left:0;width:100%;height:100%;background-size:cover;background-position:center;opacity:.15;transition:background-image 2s ease-in-out;z-index:0;}}.overlay{{position:absolute;top:0;left:0;width:100%;height:100%;background:linear-gradient(rgba(10,10,16,.6) 0%, rgba(26,26,46,.8) 100%);z-index:1;pointer-events:none;}}@media(max-width:768px){{.overlay{{background:linear-gradient(rgba(10,10,16,.8) 0%, rgba(26,26,46,.9) 100%);}}}}.content{{position:relative;z-index:2;text-align:center;}}h1{{font-family:'Orbitron',monospace;text-transform:uppercase;letter-spacing:8px;font-size:3rem;margin:0;background:linear-gradient(45deg,#667eea,#764ba2,#f093fb);background-size:400% 400%;-webkit-background-clip:text;background-clip:text;-webkit-text-fill-color:transparent;animation:gradientMove 4s ease infinite;text-shadow:0 0 20px rgba(102,126,234,.6);}}@media(max-width:768px){{h1{{font-size:2rem;letter-spacing:4px;}}}}.status{{color:#667eea;font-weight:600;letter-spacing:4px;font-size:1rem;text-transform:uppercase;margin-top:12px;font-family:'Orbitron',monospace;}}@media(max-width:768px){{.status{{font-size:.85rem;letter-spacing:2px;}}}}@keyframes gradientMove{{0%,100%{{background-position:0% 50%}}50%{{background-position:100% 50%}}}}</style></head><body><div class="main-header"><div id="bg-carousel"></div><div class="overlay"></div><div class="content"><h1>Logistics<span style="font-weight:700;"> Intelligence</span></h1><div class="status">Au service de l'optimisation des systèmes industriels et logistiques</div></div></div><script>const images=["https://images.unsplash.com/photo-1579762715215-11e8e4e570e4?w=800","https://images.unsplash.com/photo-1558618047-3c8c76bbb17e?w=800","https://images.unsplash.com/photo-1560448204-e02f11c3d0e2?w=800","https://images.unsplash.com/photo-1586528116311-82e367702ef6?w=800","https://images.unsplash.com/photo-1568605114967-8130f3a36994?w=800"];let index=0;const bgDiv=document.getElementById('bg-carousel');function changeBackground(){{bgDiv.style.backgroundImage=`url('${{images[index]}}')`;index=(index+1)%images.length;}}changeBackground();setInterval(changeBackground,3500);</script></body></html>
"""
components.html(header_code, height=180)

# GRILLE RESPONSIVE AVEC IMAGES D'ARRIÈRE-PLAN
col1, col2, col3 = st.columns([1,1,1])

with col1:
    components.html("""
    <div class="tool-card" style="background-image: linear-gradient(rgba(0,0,0,0.4), rgba(0,0,0,0.4)), url('https://images.unsplash.com/photo-1579762715215-11e8e4e570e4?w=400'); background-size: cover; background-position: center;">
        <span class="tool-icon">📦</span><div class="tool-title">PALLET OPTIMIZER</div><div class="tool-desc">Optimisation parfaite de vos palettes</div>
    </div>
    """, height=280)
    if st.button("🚀 LANCER", key="btn_app1", use_container_width=True): st.switch_page("pages/app1.py")

with col2:
    components.html("""
    <div class="tool-card" style="background-image: linear-gradient(rgba(0,0,0,0.4), rgba(0,0,0,0.4)), url('https://images.unsplash.com/photo-1558618047-3c8c76bbb17e?w=400'); background-size: cover; background-position: center;">
        <span class="tool-icon">🚛</span><div class="tool-title">CONTAINER OPTIMIZER</div><div class="tool-desc">Maximisez l'espace conteneurs</div>
    </div>
    """, height=280)
    if st.button("🚀 LANCER", key="btn_app2", use_container_width=True): st.switch_page("pages/app2.py")

with col3:
    components.html("""
    <div class="tool-card" style="background-image: linear-gradient(rgba(0,0,0,0.4), rgba(0,0,0,0.4)), url('https://images.unsplash.com/photo-1560448204-e02f11c3d0e2?w=400'); background-size: cover; background-position: center;">
        <span class="tool-icon">📊</span><div class="tool-title">VOGEL SYSTEM</div><div class="tool-desc">Algorithme approvisionnement</div>
    </div>
    """, height=280)
    if st.button("🚀 LANCER", key="btn_app3", use_container_width=True): st.switch_page("pages/app3.py")
...
with col4:
    components.html("""
    <div class="tool-card" style="background-image: linear-gradient(rgba(0,0,0,0.4), rgba(0,0,0,0.4)), url('https://images.unsplash.com/photo-1586528116311-82e367702ef6?w=400'); background-size: cover; background-position: center;">
        <span class="tool-icon">⚙️</span><div class="tool-title">MRP/CBN SOLUTION</div><div class="tool-desc">Gestion besoins matériaux</div>
    </div>
    """, height=280)
    if st.button("🚀 LANCER", key="btn_app4", use_container_width=True): st.switch_page("pages/app4.py")

with col5:
    components.html("""
    <div class="tool-card" style="background-image: linear-gradient(rgba(0,0,0,0.4), rgba(0,0,0,0.4)), url('https://images.unsplash.com/photo-1568605114967-8130f3a36994?w=400'); background-size: cover; background-position: center;">
        <span class="tool-icon">💰</span><div class="tool-title">CALCULATOR TRANSPORT</div><div class="tool-desc">Coûts transport précis</div>
    </div>
    """, height=280)
    if st.button("🚀 LANCER", key="btn_app5", use_container_width=True): st.switch_page("pages/app5.py")

with col6:
    components.html("""
    <div class="soon-card" style="background-image: linear-gradient(rgba(34,197,94,0.4), rgba(16,185,129,0.4)), url('https://images.unsplash.com/photo-1574179208501-42ad8b7d3da4?w=400'); background-size: cover; background-position: center;">
        <span class="tool-icon">📈</span><div class="tool-title">SIX SIGMA</div><div class="tool-desc">Réduction défauts & optimisation</div><div class="soon-text">🔥 COMING SOON</div>
    </div>
    """, height=280)
